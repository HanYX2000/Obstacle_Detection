...
def main(argv):
    ...
    while key != 113:  # for 'q' key
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            #添加PoseArray用于给ogm发送行人信息
            all_ped_pose = PoseArray()
            ...
            for detection in detections:
                ...
                if (label == "person"):
                    ...
                    all_ped_pose.poses.append(ped_pose)
            
            #发布出去
            pub_CamObs.publish(all_ped_pose)
            all_ped_pose.poses.clear()
...
if __name__ == "__main__":
    rospy.init_node('cam_record')
    pub_CamObs = rospy.Publisher('/Cam_Ped_poses', PoseArray, queue_size=1000)
    main(sys.argv[1:])
    rospy.spin()
